...
const float Nexus::CHUNK_SCALE = 0.05f;
...
/* Spawns a blueprint actor at the given positon and rotation */
AActor* Nexus::SPAWN_ACTOR(UClass* _bp, FVector _pos, FRotator _rot)
{
	AActor* tmp = WORLD->SpawnActor<AActor>(_bp, _pos, _rot);
	tmp->SetActorScale3D(FVector(CHUNK_SCALE, CHUNK_SCALE, CHUNK_SCALE));
	return tmp;
}